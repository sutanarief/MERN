...
const ModalEdit = props => {
  ...
  useEffect(() => {
    ...
    if (props.id) {
      fetch(`http://localhost:5000/aplikasi/${props.id}`)
        .then(response => {
          if (!response.ok) {
        ...
        .then(result => {
          result.tanggal_didirikan = result.tanggal_didirikan.split("T")[0];
          setFormData(result);
        });
    }
  }, []);
  ...
  return (
    <>
      <NavLink
        type="button"
        to={`/aplikasi/${props.id}`}
        className={props.bootstrap}
        data-bs-toggle="modal"
        data-bs-target={`#exampleModal${props.keyIndex}`}
      >
        {props.text}
      </NavLink>
      <div
        className="modal fade"
        id={`exampleModal${props.keyIndex}`}
        tabIndex="-1"
